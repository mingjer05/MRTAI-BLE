...
// Check connection to our Node.js server (Safely)
async function checkServer() {
    ...
    try {
        ...
        if (data.success) {
            serverStatusEl.innerText = `Connected to Server (v${data.version})`;
            serverStatusEl.style.color = "#4caf50";
        }
    } catch (e) {
        serverStatusEl.innerText = "(Server Offline - Build features disabled)";
...
// -------------------------------------------------------------------------
// Build Firmware via Server API (Local Only)
// -------------------------------------------------------------------------
const buildBtn = document.getElementById('buildBtn');
const bleNameInput = document.getElementById('bleNameInput');
const buildStatus = document.getElementById('buildStatus');

// Only attach listeners if the build elements exist in the HTML
if (buildBtn) {
    buildBtn.addEventListener('click', async () => {
        const bleName = bleNameInput.value.trim();
        if (!bleName) {
            alert("Please enter a Bluetooth name.");
            return;
        }

        buildStatus.innerText = "Building... This may take ~1 minute.";
        buildBtn.disabled = true;
        logMsg(`🚀 Starting remote build for: ${bleName}`);

        try {
            const response = await fetch('/api/build', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ bleName })
            });
            ...
            try {
                ...
                throw new Error(`Server returned HTML instead of JSON. Check if server.js is running locally.`);
            }

            if (response.ok) {
                logMsg("✅ " + result.message);
                buildStatus.innerText = "Success! You can now Flash (USB).";
                buildStatus.style.color = "#4caf50";
            } else {
                throw new Error(result.message || "Build failed");
            }
        } catch (err) {
            logMsg("❌ Build Error: " + err.message);
            buildStatus.innerText = "Error: " + err.message;
            buildStatus.style.color = "#f44336";
        } finally {
...
if (flashBtn && fileInput) {
    flashBtn.addEventListener('click', async () => {
        ...
        if (fileInput.files.length > 0) {
            ...
            // Otherwise, try to fetch the most recently built firmware from the server
            logMsg("Fetching latest build from server...");
            try {
                const response = await fetch('/api/firmware');
                if (!response.ok) throw new Error("No server binary found. Build first or select a file.");
                const blob = await response.blob();
                fileName = "server_firmware.bin";
                fileData = await new Promise((resolve) => {
                    const reader = new FileReader();
                    reader.onload = (e) => resolve(e.target.result);
